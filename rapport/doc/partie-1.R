...
cor(recettes.pays)    # correlation entre les ingredients
...
acp.recettes.pays <- prcomp(recettes.pays)  # ACP - variables = ingredients

plot(acp.recettes.pays$x[,1:2])
text(acp.recettes.pays$x[,1:2], row.names(recettes.pays), pos=3)

acp.recettes.pays$sdev # racine carree des valeurs absolues
...
# on retrouve a peu pres les memes resultat
# on peut considerer les valeurs d'indice superieur a 26 egales a 0, toutes ces valeurs sont inferieur a 10^-16
# elles ne sont du qu'a des approximations, en effet on part d'une matrice de rang maximum 26
...
# plot des anciens axes dans le nouveau - a voir - certainement ilisible

## 3 - Analyse ascendante hierarchique
...
## 5 - Classification geographique des origines
...
# TODO : similarites - disimilarites
